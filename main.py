...
async def load_cogs():
    try:
        await bot.load_extension("module.Basic_module")  # module 폴더에서 로드
        print("✅ BASIC MODULE 로드 성공")
    except Exception as e:
        print(f"⚠️ BASIC MODULE 로드 실패: {e}")

    try:
        await bot.load_extension("module.music_module")  # module 폴더에서 로드
        print("✅ MUSIC MODULE 로드 성공")
    except Exception as e:
        print(f"⚠️ MUSIC MODULE 로드 실패: {e}")

    try:
        await bot.load_extension("module.random_module")  # module 폴더에서 로드
        print("✅ RANDOM MODULE 로드 성공")
    except Exception as e:
        print(f"⚠️ RANDOM MODULE 로드 실패: {e}")

    try:
        await bot.load_extension("module.weather_module")  # module 폴더에서 로드
        print("✅ WEATHER MODULE 로드 성공")
    except Exception as e:
        print(f"⚠️ WEATHER MODULE 로드 실패: {e}")

    # try:
    #     await bot.load_extension("module.lunch_module")  # module 폴더에서 로드
